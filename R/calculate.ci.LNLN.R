...
#   as used during the estimation procedure. It has to be TY*(m+1)-dimensional, even for
#   fix.mu==T.
...
#   (mu_type_1_gene_1, mu_type_1_gene_2, ..., mu_type_2_gene_1, mu_type__gene_2, ...)
   ...
   if (fix.mu) {
      mu.in.parameter <- parameter[TY:(length(parameter)-1)]
   ...
   hesse <- hessian(func=to.minimize,x=parameter.trans)
   ...
   CI <- matrix(NA,ncol=2,nrow=TY*(m+1))
